...
class WifiBloc extends Bloc<WifiEvent, WifiState> {
  ...
  WifiBloc() : super(WifiStateLoading()) {
    on<WifiEventLoad>((event, emit) => _mapLoadToState(event, emit));
    on<WifiEventStartProvisioning>((event, emit) => _mapProvisioningToState(event, emit));
  }

  Future<void> _mapLoadToState(WifiEventLoad event, Emitter<WifiState> emit) async {
